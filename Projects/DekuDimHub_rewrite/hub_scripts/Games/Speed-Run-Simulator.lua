-- "https://raw.githubusercontent.com/1201for/V.G-Hub-Part-3/main/Speed-Run-Simulator"

if(not game:IsLoaded())then game.Loaded:Wait()end;
if(game.PlaceId~=7047488135 or nil~=getgenv()["MiJy+vyRLYC=r+E2Wgn4:B~nCaN&[9sb"])then return end;
getgenv()["MiJy+vyRLYC=r+E2Wgn4:B~nCaN&[9sb"] = true;
...
local Name = "Speed Shit" .. " " .. Player.Name .. ".json"
...
local Run =
    CoreGui.RobloxPromptGui.promptOverlay.ChildAdded:Connect(
    function(v)
...
local Eggs = {}
for i,v in pairs(require(ReplicatedStorage.Modules.EggRarities)) do
...
local Tab1 = Window:CreateTab("Speed Simulator Something idfk")
...
-------------
local Toggle1 = Section1:CreateToggle("Auto Speed", Settings.Speed, function(State)
...
local Toggle1 = Section1:CreateToggle("Auto Egg", Settings.Eggs, function(State)
Settings.Eggs = State
RunService.Heartbeat:Connect(function()
    if Settings.Eggs then
        game:GetService("ReplicatedStorage").Remotes.CanBuyEgg:InvokeServer(Settings.Egg)
    end 
end)
end)


local Dropdown = Section1:CreateDropdown("Eggs", Eggs, function(Name)
	Settings.Egg = Name
end)

local Toggle1 = Section1:CreateToggle("Auto Orbs", Settings.Orb, function(State)
Settings.Orb = State
RunService.Heartbeat:Connect(
    function()
        if Settings.Orb then
            for _, v in pairs(game:GetService("Workspace").OrbSpawns:GetChildren()) do
