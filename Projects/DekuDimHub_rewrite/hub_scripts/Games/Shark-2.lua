-- "https://raw.githubusercontent.com/1201for/V.G-Hub-Part-3/main/Shark-2"

if(not game:IsLoaded())then game.Loaded:Wait()end;
if(game.PlaceId~=8908228901 or nil~=getgenv()[":5lsRkluyvj0eac_n+8*0[9;q_kTtar*"])then return end;
getgenv()[":5lsRkluyvj0eac_n+8*0[9;q_kTtar*"] = true;
...
function ClosestSharkToCurser()
    local MaxDistance, Closest = math.huge
    for i,v in pairs(Folder:GetChildren()) do
        if v then
            local H = v.PrimaryPart
            if H then 
                ...
                if Vis then
                    ...
                    if Dist < MaxDistance and Dist <= Settings.Aimbot.FOV then
                        MaxDistance = Dist
                        Closest = v
                    end
                end
    ...
    function()
        pcall(
            function()
                ...
                ClosestPlayer = ClosestSharkToCurser()
                if ClosestPlayer and Settings.Method == "FirstPerson" then
                    Workspace.CurrentCamera.CFrame =
                        CFrame.new(Workspace.CurrentCamera.CFrame.p, ClosestPlayer.PrimaryPart.CFrame.p)
                end
                if ClosestPlayer and Settings.Method == "ThirdPerson" then
                      local Mouse = game:GetService("UserInputService"):GetMouseLocation()
                local TargetPos = game.workspace.Camera:WorldToViewportPoint(ClosestPlayer.PrimaryPart.Position)
                mousemoverel(
...
local Tab1 = Window:CreateTab("SharkBite 2")
...
local Dropdown1 = Section1:CreateDropdown("Aimbot Method", {"FirstPerson","ThirdPerson"}, function(String)
...
local Toggle1 = Section1:CreateToggle("Shark", Settings.Blue, function(State)
    Settings.Blue = State
    ESP.Skark = Settings.Blue

ESP:AddObjectListener(game:GetService("Workspace").Sharks, {
...
local Button1 = Section1:CreateButton("GunMods", function()
