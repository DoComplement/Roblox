...
workspace.CurrentCamera.trash.ChildAdded:connect(
    function(v)
        if getgenv().Silent then
        v.CFrame = CFrame.new(nearest().Character.HumanoidRootPart.Position)
        end 
    end
)
end)
...
game:GetService("RunService").Stepped:connect(
    function()
        if knife then
            ...
            game:GetService("ReplicatedStorage").forhackers:InvokeServer(
                "throw",
                CFrame.new(
                    Vector3.new(0,0,0),
                    Vector3.new(0,0,0)
            ...
            for i,v in pairs(game:GetService("Workspace").bags:GetChildren()) do
