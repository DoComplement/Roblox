-- "https://raw.githubusercontent.com/1201for/V.G-Hub-Part-3/main/Reaper"

if(not game:IsLoaded())then game.Loaded:Wait()end;
if(game.PlaceId~=7056922815 or nil~=getgenv()["bnY_C=xzM9S(t^HV@TEE2t?D+dA3eNSY"])then return end;
getgenv()["bnY_C=xzM9S(t^HV@TEE2t?D+dA3eNSY"] = true;
...
Hentai =
    hookfunction(
    getrawmetatable(game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart").Position).__index,
    function(i, v)
        if getcallingscript().Name == "ClientHandler" and v:lower() == "magnitude" then
...
for i, v in pairs(game:GetService("Workspace").Living:GetChildren()) do
    if v:FindFirstChild("Value") and not table.find(mobs, v.Name) then
...
local Toggle1 = Section1:CreateToggle("Auto Attack", Settings.Hentai, function(State)
Settings.Hentai = State
spawn(
    function()
        while wait(0.2) and Settings.Hentai do
...
local Toggle1 = Section1:CreateToggle("Auto Mob", Settings.OhNo, function(State)
Settings.OhNo = State
RunService.Stepped:connect(
    function()
        if Settings.OhNo then
...
spawn(
    function()
        while Settings.OhNo do
            wait()
            pcall(
                function()
                    for i, v in pairs(game:GetService("Workspace").Living:GetChildren()) do
                        if
                            v.Name == Settings.Mob and v:FindFirstChildOfClass("Humanoid") and
...
local Dropdown1 = Section1:CreateDropdown("Bosses", mobs, function(Name)
	Settings.Mob = Name
end)
            ...
            pcall(
                function()
                    for i, v in pairs(game:GetService("Workspace").NPCs:GetChildren()) do
                        local v =
                            v:FindFirstChild("Dialogue"):FindFirstChild("1"):FindFirstChild("2"):FindFirstChild(
                            "Continue"
                        )
                        if
                            v.Name == ("Quest") and v.Value ~= "" and
                                not game:GetService("Players").LocalPlayer.PlayerGui.HUD.QuestsFrame2:FindFirstChild(
                                    v.Value
                                )
                         then
                            game:GetService("ReplicatedStorage").Remotes.TakeQuest:FireServer(
                                v.Value,
                                v.Parent.Parent.Parent.Parent
                            )
                            wait(3)
