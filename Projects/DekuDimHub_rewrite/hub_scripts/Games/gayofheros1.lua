-- "https://raw.githubusercontent.com/1201for/V.G-Hub/main/gayofheros1"

if(not game:IsLoaded())then game.Loaded:Wait()end;
if(game.PlaceId~=4866692557 or nil~=getgenv()["aRqX^*ozD)?f&]1YfH8orlghg.N~d1C@"])then return end;
getgenv()["aRqX^*ozD)?f&]1YfH8orlghg.N~d1C@"] = true;
...
Table = {}
a = require(game:GetService("ReplicatedStorage").Modules.Upgrades)
for i,v in pairs(a) do
table.insert(Table,i)
end 

SS = {"Thug",'Civilian','Police'}

local function CharacterAdded(v)
    local Food = false
    local LastPostion
    v:WaitForChild("Humanoid").HealthChanged:Connect(
        function(f)
            if f <= 40 and not Food then
                Food = true
                LastPostion = v:WaitForChild("HumanoidRootPart").CFrame
                v:WaitForChild("HumanoidRootPart").CFrame = CFrame.new(-1726.80713, 94.5901413, -1368.02319)
            elseif f > 40 and Food then
                Food = false
                if LastPostion then
                    v:WaitForChild("HumanoidRootPart").CFrame = LastPostion
                    LastPostion = nil
                end
            end
        end
...
function WalkSpeed_System()
    if Debounce == false then
        getgenv().WalkSpeed = 200
        game:GetService("Players").LocalPlayer.Character.Humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(
            function()
...
local Tab1 = Window:CreateTab("Age of Heroes")
...
local Toggle1 = Section1:CreateToggle("Rep Farm", nil, function(State)
tt = State
spawn(
    function()
        while tt do
    ...
    pcall(
        function()
            for i, v in pairs(game.Workspace:GetChildren()) do
                if v.ClassName == "Model" and v.Name == SS and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
...
local Toggle1 = Section1:CreateToggle("Life Saver", nil, function(State)
gogo = State
if gogo then
    CharacterAdded(
...
local Toggle1 = Section1:CreateToggle("Press X WalkSpeed", nil, function(State)
getgenv().Waiting = State
game:GetService("Players").LocalPlayer:GetMouse().KeyDown:connect(
    function(v)
        if getgenv().Waiting and v == "x" then
            WalkSpeed_System()
        end
    end
)
...
local Toggle1 = Section1:CreateToggle("Auto Orbs", nil, function(State)
wow = State
            ...
            pcall(
                function()
                    firetouchinterest(
                        ...
                        game:GetService("Workspace").ExperienceOrbs.experienceOrb,
                        0
                    )
                end
            )
...
local Toggle1 = Section1:CreateToggle("Auto UpgradeAbility", nil, function(State)
ab = State
while ab do
    wait()
    game:GetService("ReplicatedStorage").Events.UpgradeAbility:InvokeServer(Table)
end
end)

local Dropdown1 = Section1:CreateDropdown("Select Ability")
...
local Toggle1 = Section1:CreateToggle("Telekinesis No Cooldown", nil, function(State)
a = require(game.Players.LocalPlayer.Character.telekinesisClient)
if nocool then
    a.coolDown = 0
    a.startCost = 0
    a.loopCost = 0
