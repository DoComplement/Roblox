...
if(game.PlaceId~=292439477 or nil~=getgenv()["Jy8!TSAnuOkU!(ow^w)@3c(`+9yVPCce"])then return end;
getgenv()["Jy8!TSAnuOkU!(ow^w)@3c(`+9yVPCce"] = true;
...
function GetClosestToCuror()
    Closest = math.huge
    Target = nil
    for i, v in pairs(Workspace.Players[tostring(GetTeam())]:GetChildren()) do
        if v ~= Player and v and v:FindFirstChild("Torso") then
            Point, OnScreen = Workspace.CurrentCamera:WorldToViewportPoint(v.Torso.Position)
            if OnScreen and NotObstructing(v.Torso.Position, {Player.Character, v}) then
                Distance =
                    (Vector2.new(Point.X, Point.Y) - Vector2.new(Player:GetMouse().X, Player:GetMouse().Y)).magnitude
...
RunService.RenderStepped:Connect(
    function()
        ...
        if ClosestPlayer then
            ...
            mousemoverel(
                (TargetPos.X - Mouse.X) * 0.09,
                (TargetPos.Y - Mouse.Y) * 0.09
            )
        end
    end
)
            ...
            while Aim do
                ...
                if
                    game:GetService("Players").LocalPlayer:GetMouse().Target and
                        Workspace.Players[tostring(GetTeam(Null))]:FindFirstChild(
...
RunService.RenderStepped:Connect(
    function()
        if Workspace.Camera:FindFirstChild("Right Arm") and Settings.RainBow then
