-- "https://raw.githubusercontent.com/1201for/V.G-Hub/main/Magnet%20Simulator"

if(not game:IsLoaded())then game.Loaded:Wait()end;
if(game.PlaceId~=3486025575 or nil~=getgenv()["KcSc7_S^uDh&3@v)Bp.aGQM`!FuA%-:s"])then return end;
getgenv()["KcSc7_S^uDh&3@v)Bp.aGQM`!FuA%-:s"] = true;
...
Eggs = {}
for i, v in pairs(game:GetService("Workspace").EggHolder:GetChildren()) do
    if v:FindFirstChild("EggHolder") then
        table.insert(Eggs, v.EggHolder.gui.SurfaceGui.eggname.Text)
    end
end
Tools = {}
...
pcall(function()
game:GetService("Players").LocalPlayer.PlayerGui.GameHUD.FullBackpack:remove()
end)
...
while jj do
    wait()
    game:GetService("ReplicatedStorage").Events.MagnetEvents.requestGrab:FireServer(
        "1040000000",
        game.ReplicatedStorage.Tools[Tools]
    )
end
 
end)
local Dropdown1 = Section1:CreateDropdown("Select Magnet")
Dropdown1:AddToolTip("Select Magnet")
...
local Button1 = Section1:CreateButton("Buy Backpack", function()
game:GetService("ReplicatedStorage").Events.ShopEvents.requestBuy:FireServer(Backpacks,"Backpack")
end)
...
getgenv().Eg = State
while wait() and getgenv().Eg do 

game:GetService("ReplicatedStorage").PetEvents.requesthatch:FireServer(Eggs)
end
end)
