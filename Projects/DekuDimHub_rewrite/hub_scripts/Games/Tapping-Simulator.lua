-- "https://raw.githubusercontent.com/1201for/V.G-Hub-Part-3/main/Tapping-Simulator"

if(not game:IsLoaded())then game.Loaded:Wait()end;
if(game.PlaceId~=9498006165 or nil~=getgenv()["B?3*:e`#*tQ-@E5~#RNhlBq`7@9~S9fK"])then return end;
getgenv()["B?3*:e`#*tQ-@E5~#RNhlBq`7@9~S9fK"] = true;

-- No One Asked But Who Cared wait shit all caps tf
...
for i,v in pairs(Workspace.Shops:GetChildren()) do
    if v:IsA("Model") and not v.Name:find("Robux") and not table.find(Eggs,v.Name) then
        table.insert(Eggs,v.Name)
    end 
end 
local Islands = {}
for i,v in pairs(Workspace.Islands:GetChildren()) do
    if v:IsA("Model") and not table.find(Islands,v.Name) then
...
local Name = "Tapping Simulator!.json"
...
local Toggle1 = Section1:CreateToggle("Auto Click", Settings.AutoClick, function(State)
Settings.AutoClick = State
spawn(function()
    while wait() and Settings.AutoClick do 
...
local Dropdown3 = Section1:CreateDropdown("Teleport To Island", Islands, function(Name)
    Player.Character.HumanoidRootPart.CFrame = game:GetService("Workspace").Islands[Name].Path:GetModelCFrame() 
end)
