-- "https://raw.githubusercontent.com/1201for/V.G-Hub-Part-3/main/Demon-Soul-Simulator"

if(not game:IsLoaded())then game.Loaded:Wait()end;
if(game.PlaceId~=8069117419 or nil~=getgenv()["o-5=q?*^`JhbW-v;NZdA@Aay6bYVWq!0"])then return end;
getgenv()["o-5=q?*^`JhbW-v;NZdA@Aay6bYVWq!0"] = true;
...
local Name = "Demon-Soul-Simulator.json"
...
local Ghosts = Workspace.GhostPos
...
local Levels = {}
for i, v in pairs(Ghosts:GetChildren()) do
...
local function getNearestMob()
    ...
    local Target
    for x, d in pairs(Ghosts:GetChildren()) do
        if d:IsA("Folder") and d.Name == Settings.Level then
...
local Tab1 = Window:CreateTab("Demon-Soul-Simulator")
...
spawn(
    function()
        while Settings.Start do
            ...
            pcall(
                function()
                    Character.HumanoidRootPart.CFrame = getNearestMob().HumanoidRootPart.CFrame *CFrame.new(0,0,3) wait(1)
...
local Dropdown1 = Section1:CreateDropdown("Demon Levels", Levels, function(String)
    Settings.Level = String
end)


local Toggle1 = Section2:CreateToggle("Auto Roll Avatar", Settings.Easy, function(State)
Settings.Easy = State
spawn(
    function()
        while Settings.Easy do
            wait(0.1)
            pcall(
                function()
                    ReplicatedStorage.RemoteEvents.DrawRole:FireServer(false)
                end
            )
        end
    end
