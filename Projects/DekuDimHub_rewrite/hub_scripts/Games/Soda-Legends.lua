-- "https://raw.githubusercontent.com/1201for/V.G-Hub/main/Soda-Legends"

if(not game:IsLoaded())then game.Loaded:Wait()end;
if(game.PlaceId~=6028934209 or nil~=getgenv()["*x7Ny6)$s`?A5`*l%y+AmMwhF=taE$?1"])then return end;
getgenv()["*x7Ny6)$s`?A5`*l%y+AmMwhF=taE$?1"] = true;
...
local Toggle1 = Section1:CreateToggle("Auto Hoops", nil, function(State)
v3rm1 = State
game:GetService("RunService").Stepped:connect(
    function()
        if v3rm1 then
            for _, v in pairs(
                game:GetService("Workspace").Worlds.VoidIsland.ParkourElements.Utility.CoinRings:GetChildren()
            ) do
                if v:IsA("Model") then
                    firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, v.Part, 0) --0 is touch
                    wait()
                    firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, v.Part, 1) -- 1 is untouch
                end
            end
        end
    end
...
local Toggle1 = Section1:CreateToggle("Auto Eggs", nil, function(State)
getgenv().Ok = State
while wait() and getgenv().Ok do
    game:GetService("ReplicatedStorage").Events.Pets.RollForPet:FireServer(Eggs, "1")
end
end)
...
local Toggle1 = Section1:CreateToggle("Auto Coins", nil, function(State)
v3rm2 = State
game:GetService("RunService").Stepped:connect(
    function()
        if v3rm2 then
            ...
            for i, v in pairs(game:GetService("Workspace").Drops:GetChildren()) do
                if v:IsA("Part") and v:FindFirstChildOfClass("Model") then
                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame =
                        v:FindFirstChildOfClass("Model").MainPart.CFrame
                end
            end
        end
    end
...
for _,v in pairs(game:GetService("Workspace").GameMechanics.Chests:GetDescendants()) do
