...
do
    ...
    local cf = CFrame.new(147.143021, 29.7740364, -13.7545681)
    Section1:CreateToggle("AutoFarm", nil, function(State)
        wait1 = State;
        if(State)then
            f(character);
            con = Player.CharacterAdded:Connect(f);
            while(task.wait()and wait1)do
                rootPart.CFrame = cf;
            end;
        else
            con:Disconnect();
...
do
    local con = nil;
    local function f()
        local M = Workspace.Map:FindFirstChildOfClass("Model"):
        if(nil~=M:FindFirstChild("Water"))then
            M.Water:Destroy();
        end;
    end;
    Section1:CreateToggle("Water GodMode", nil, function(State)
        if(State)then
...
do
    local PowerUpsRE = game:GetService("ReplicatedStorage").ReplicationManagers.PowerUpsRE;
    local FireServer = PowerUpsRE.FireServer;

    local function setup_autobuy(thing)
        local t = false;
        return function(State)
            t = State;
            while(task.wait()and t)do
                FireServer(PowerUpsRE, "BuyPowerUps", thing, 1);
            end;
        end;
    end;

    Section1:CreateToggle("Auto Buy Healing", false, setup_autobuy("Healing")end);
    Section1:CreateToggle("Auto Buy Gravity Boots", false, setup_autobuy("Gravity Boots",1)end);
    Section1:CreateToggle("Auto Buy Jump Boost", false, setup_autobuy("Jump Boost")end);
    Section1:CreateToggle("Auto Buy Parachute", false, setup_autobuy("Parachute")end);
    Section1:CreateToggle("Auto Buy Ghostify", false, setup_autobuy("Ghostify")end);
end;
