...
if(table.find(({3941862963,34764673}),LocalPlayer.UserId)~=nil)then
	_G.EGG = "Autumn Egg 2";
end;
...
local function GetItemPower(ID, Item, Data, Calculator, Event)
	if(Data.Vanity~=nil)then
		...
		if(Event~=nil and(not Tags or Tags[1]~=Event))then return end;
...
do
	...
	autoEgg.Event:Connect(function()
		...
		if(_G.FARM_EGGS)then InvokeServer(Events[3],_G.EGG,HatchType)end;
	end);
